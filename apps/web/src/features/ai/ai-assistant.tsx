...
import { AlertTriangle, Lock, RefreshCw, Send, Sparkles, Trash2, User } from "lucide-react";
...
import { aiChatStorageKey, clearAiChat } from "./storage";
import { AI_TOOL_LABEL, type AiAnswer, type AiChatMessage } from "./types";
...
/** Max messages of prior context sent with a follow-up (server re-clamps). */
...
/** Restore a user's prior conversation, validating each entry's shape. */
function loadInitialTurns(userId?: string): Turn[] {
  if (!userId || typeof window === "undefined") return [];
  try {
    const raw = window.localStorage.getItem(aiChatStorageKey(userId));
    if (!raw) return [];
    ...
    return parsed.filter(
      (t): t is Turn =>
        ...
        ((t as Turn).status === "done" || (t as Turn).status === "error"),
    );
  } catch {
    return [];
  }
...
export function AiAssistant() {
  ...
  // Persisted per authenticated user, restored on mount (survives refresh).
  const [turns, setTurns] = useState<Turn[]>(() => loadInitialTurns(user?.id));
  ...
  // Persist the conversation, scoped to the signed-in user.
  useEffect(() => {
    if (!user?.id || typeof window === "undefined") return;
    const key = aiChatStorageKey(user.id);
    try {
  ...
  /** Recent completed Q→A pairs as multi-turn context (excluding a retried turn). */
  const buildHistory = (excludeId?: string): AiChatMessage[] =>
    turns
      .filter((t) => t.id !== excludeId && t.status === "done" && t.answer)
      .flatMap<AiChatMessage>((t) => [
  ...
  const ask = async (question: string, existingId?: string) => {
    ...
    const history = buildHistory(existingId);
    setTurns((prev) =>
      existingId
    ...
    setInput("");
    try {
      const answer = await aiQuery.mutateAsync({ question: trimmed, history });
  ...
  const clearChat = () => {
    setTurns([]);
    if (user?.id) clearAiChat(user.id);
  };
  ...
  return (
    <div className="flex h-[calc(100vh-12rem)] flex-col gap-3">
      {turns.length > 0 ? (
        <div className="flex items-center justify-between">
          <span className="font-label-sm text-[11px] text-on-surface-variant">
            {turns.length} {turns.length === 1 ? "message" : "messages"} · saved to this device
          </span>
          <Button variant="ghost" size="sm" onClick={clearChat} disabled={aiQuery.isPending}>
          ...
          <div className="flex h-full flex-col items-center justify-center gap-5 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="max-w-md font-body-md text-on-surface-variant">
                ...
                over your shop&apos;s data, and remembers the conversation so you can ask follow-ups.
              </p>
            </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-start gap-2 rounded-2xl rounded-tl-sm border border-error-container bg-error-container/30 px-4 py-2.5 text-on-error-container">
                          ...
                          <span className="font-body-sm text-body-sm">
                            {turn.error || "The assistant is unavailable right now. Please try again."}
                          </span>
                        </div>
      ...
      {approved ? (
        <div className="space-y-2">
          {/* Persistent quick actions — predefined prompts stay one tap away. */}
          {turns.length > 0 ? (
            <div className="no-scrollbar flex gap-2 overflow-x-auto pb-1">
              {SUGGESTIONS.map((s) => (
                <button
                  ...
                  className="shrink-0 rounded-full border border-outline-variant bg-surface px-3 py-1.5 font-label-sm text-[11px] text-on-surface-variant transition-colors hover:border-primary hover:text-primary disabled:opacity-50"
                >
                  {s}
                </button>
              ))}
          ...
          <form onSubmit={onSubmit} className="flex items-end gap-2">
            <Textarea
              ...
              placeholder="Ask a follow-up about expiries, stock, sales…"
              rows={1}
